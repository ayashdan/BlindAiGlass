...
## The social pass: weekly league, invites, hype feed, Plus preview

Built from `docs/social-growth-review.md` (items 1-7 of the ranked plan).

**Setup — run `supabase/migrations/0029_social_engagement.sql` in Supabase
SQL Editor.** One new cron was added to `vercel.json`
(`/api/cron/league-finish`, Mondays 00:05 UTC) — redeploy so it registers.
No new env vars.

- **Weekly friends league** — the Friends tab now ranks you + your friends
  by **XP earned this week** (Monday reset, UTC), with an "All time"
  toggle. Only base workout XP counts, max 3 workouts a day, so the score
  measures showing up, not grinding — and bounds what fake logging can
  buy. The dashboard rival card and the leaderboard's default tab are
  weekly too. Winners get a push + a 👑 on last week's crown; the Monday
  cron records history in `league_weeks` and resets scores (reads are
  lazy-reset too, so a missed cron never shows wrong numbers).
- **Invite loop** — everyone has an invite link on the Friends tab
  (`/signup?ref=yourname`). Signing up through it **auto-friends** the two,
  so the invitee's day one already has a rival and a league. When a recruit
  logs their **3rd** workout, the inviter unlocks the 🎖️ **Recruiter**
  title (cosmetic only — never XP, so the league stays clean) and gets a
  push.
- **Hype feed** — logging a workout drops an event your friends see on the
  Friends tab (with level-up / PR / achievement badges). They can tap 🔥
  **Hype** once per event; you get a push when someone hypes you. No
  comments or free text — nothing to moderate. Old events are pruned after
  14 days by the Monday cron.
- **Shop detox → Forge Plus preview** — purchasable XP, XP boosts, paid
  chests, and Streak Restore are gone (pay-to-win + loot-box + pain-tax
  problems; see the review). `/shop` now previews **Forge Plus**
  ($2.99/mo, planned): animated borders, deep stats, custom quest slot,
  +1 freeze cap, supporter badge — plus a **"Notify me"** button that
  measures real demand (count shown in `/admin/analytics`) before any
  payment processor exists. A "Free forever" card states what will never
  be paywalled.
- **Share cards** — Share buttons now attach a branded 1080×1080 image
  (generated by `/api/share-card`, no external service) when the device
  supports sharing files; plain text otherwise.
- **Design pass** — the dashboard header's six utility chips moved to a
  Settings section on `/profile` (⚙️ links there); accent colors now have
  fixed meanings (amber = rewards/progression, emerald = quests, sky =
  streak/recovery, rose = competition); big numbers use thousands
  separators; Friends/Leaderboard empty states sell an action instead of
  a grey line; light-mode muted text darkened to clear WCAG AA.
